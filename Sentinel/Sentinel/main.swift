			...
			if configuration.lowercased() == "debug" {
				if let debug = settings["debug"] as? [[String: Any]] {
					for instruction in debug {
						if let rule = instruction["rule"] as? String, let message = instruction["message"] as? String, let type = instruction["type"] as? String, let file = instruction["file"] as? String {
